...
trait BestEntityMentionsFound extends BestEntityMentionResolvedDocument[BestEntityMention]{
  ...
  private lazy val NERAnnotatedDoc = {
    val doc = new Annotation(this.text)
      ...
      if(!relevantTokens.isEmpty){
        ...
        for(tok <- relevantTokens.tail){
          if(lastTok.index() == (tok.index()-1)){
            nerType += tok.originalText()
          }
          else{
            nerTypes = nerTypes :+ nerType
            nerType = ""
          }
          lastTok = tok
        }
      ...
      if(!relevantTokens.isEmpty){
        ...
        for(tok <- relevantTokens.tail){
          if(lastTok.index() == (tok.index()-1)){
            ...
            entities = entities :+ Entity(nerType,lastTok.endPosition()-nerType.length()+1,lastTok.ner())
            nerType = ""
          }
          lastTok = tok
        }
  ...
  val bestEntityMentions = {
    documentEntities.map(entity =>
      BestEntityMention(this.text,entity.offset,bestEntityMentionFinder.findBestEntity(entity.name, entity.entityType, entity.offset, this.text, this.namedEntityCollection)))
  }
...
class BestEntityMentionFinderOriginalAlgorithm extends BestEntityMentionFinder{
  ...
  private val stopWords = {
    val url = getClass.getResource("/edu/washington/knowitall/entitymentionfinder/eval/stopwords.txt")
  ...
  override def findBestEntity(entityName: String, entityType: String, begOffset: Int, docText:String, namedEntityCollection: NamedEntityCollection): String = {
    ...
    alternateName match{
      case originalName => {
        alternateName = findBestOrganizationString(originalName,namedEntityCollection.organizations,docText,begOffset,namedEntityCollection)
        if(alternateName == originalName){
            alternateName = findBestLocationString(originalName,namedEntityCollection.locations,docText,begOffset)
        }
        if(alternateName == originalName){
          alternateName = findBestPersonString(originalName,namedEntityCollection.people,docText,begOffset,false)
        }
      }
      case _ => {}
    }
